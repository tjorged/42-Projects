...
void	philo_eats(t_philo *philo)
{
	...
	{
		pthread_mutex_lock(&philo->table->mutex);
		if (im_dead(philo, philo->table->current_time))
		{
			pthread_mutex_unlock(&philo->table->mutex);
			break;
		}
		else if (philo->table->current_time >= philo->food_deadline)
		{
			pthread_mutex_unlock(&philo->table->mutex);
			philo_sleeps(philo);
			break ;
...
void	philo_sleeps(t_philo *philo)
{
	...
	while (1)
	{
		...
		else if (philo->table->current_time >= philo->sleep_deadline)
		{
			pthread_mutex_unlock(&philo->table->mutex);
			philo_thinks(philo);
			break;
...
void	philo_thinks(t_philo *philo)
{
	...
	while (1)
	{
		...
		else if (philo->table->current_time >= philo->thinking_deadline)
		{
			pthread_mutex_unlock(&philo->table->mutex);
			break;
		}
